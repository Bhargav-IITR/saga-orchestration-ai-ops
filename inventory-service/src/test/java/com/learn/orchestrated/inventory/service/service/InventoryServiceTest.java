...
class InventoryServiceTest {
    ...
    @Test
    void shouldReleaseInventoryOnlyOnceForRemediationRedelivery() {
        OrderInventory orderInventory = OrderInventory.builder()
                .inventory(inventory)
                .oldQuantity(10)
                .newQuantity(5)
        ...
        inventoryService.releaseInventoryForRemediation(event);
        inventoryService.releaseInventoryForRemediation(event);

        assertTrue(orderInventory.isReleased());
        assertEquals(10, inventory.getAvailable());
        verify(inventoryRepository, times(1)).save(inventory);
        verify(orderInventoryRepository, times(1)).save(orderInventory);
    }
