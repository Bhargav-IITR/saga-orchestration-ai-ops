...
class PaymentServiceTest {
    ...
    @Test
    void shouldApplyIdempotentPartialAutomatedRefund() {
        payment.setStatus(PaymentStatus.SUCCESS);
        when(paymentRepository.findByOrderIdAndTransactionId(any(), any()))
                .thenReturn(Optional.of(payment));

        paymentService.applyAutomatedRefund(event, new BigDecimal("25"));
        paymentService.applyAutomatedRefund(event, new BigDecimal("25"));

        assertEquals(PaymentStatus.PARTIAL_REFUND, payment.getStatus());
        assertEquals(5.0, payment.getRefundedAmount());
        verify(paymentRepository, times(1)).save(payment);
    }

    @Test
    void shouldRejectAutomatedRefundAboveSecurityLimit() {
        payment.setStatus(PaymentStatus.SUCCESS);
        ...
        assertThrows(Exception.class,
                () -> paymentService.applyAutomatedRefund(event, new BigDecimal("100")));
