...
public class PaymentService {
    ...
    @Transactional
    public void applyAutomatedRefund(Event event, BigDecimal percentage) {
        if (percentage == null || percentage.signum() <= 0
                || percentage.compareTo(new BigDecimal("100")) > 0) {
            throw new ValidationException("Refund percentage must be greater than 0 and at most 100.");
        }
        ...
        if (payment.getStatus() != PaymentStatus.SUCCESS
            ...
            throw new ValidationException("Only a captured payment can be refunded.");
        }

        BigDecimal targetRefund = BigDecimal.valueOf(payment.getTotalAmount())
                .multiply(percentage)
                .divide(new BigDecimal("100"), 2, RoundingMode.HALF_UP);
        if (targetRefund.compareTo(new BigDecimal("500.00")) > 0) {
            throw new ValidationException("Automated refund exceeds the hard limit of R$ 500.00.");
        }
        BigDecimal alreadyRefunded = BigDecimal.valueOf(payment.getRefundedAmount());
        if (targetRefund.compareTo(alreadyRefunded) <= 0) {
            log.info("Automated refund already applied for order {} at target R$ {}",
                    event.getOrderId(), targetRefund);
            return;
        }

        payment.setRefundedAmount(targetRefund.doubleValue());
        payment.setStatus(targetRefund.compareTo(BigDecimal.valueOf(payment.getTotalAmount())) >= 0
                ? PaymentStatus.REFUND : PaymentStatus.PARTIAL_REFUND);
        save(payment);
        log.info("Applied automated refund for order {}: R$ {} ({}%)",
                event.getOrderId(), targetRefund, percentage);
    }
