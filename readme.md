...
|-------|---------|-------------|---------|
| **OperationsAgent** | Kafka `notify-ending` (FAIL) | Diagnoses failures and emits bounded remediation commands | pgvector + PostgreSQL + Redis |
...
| `DIAGNOSTIC_CACHE_TTL` | `PT24H` | No | ISO-8601 TTL for semantic diagnostic cache entries |
...
The OperationsAgent vectorizes new saga incident patterns into **pgvector** using Ollama's `nomic-embed-text` model. When a saga fails, the agent searches for similar past incidents to enrich its diagnosis. Exact trace-cache duplicates are not inserted again.

### Semantic diagnostic cache and compensation retry engine

Failed saga diagnostics now use Redis before pgvector and the language model. The cache stores the structured diagnostic under both a SHA-256 hash of the normalized failure history and a hash of the query embedding. Exact failure patterns therefore avoid both embedding generation and model invocation; embedding matches avoid vector retrieval and model invocation. Cache failures are fail-open to the existing RAG path, and `DIAGNOSTIC_CACHE_TTL` controls the entry lifetime (default `PT24H`).

The OperationsAgent emits a versioned `RemediationCommand` JSON document to `saga-remediation-dlq-retry`. A separate listener validates the immutable source event and applies hard security bounds before forwarding an allowlisted action:

```text
notify-ending (FAIL)
  -> Redis cache or pgvector + OperationsAgent
  -> saga-remediation-dlq-retry
  -> bounded policy + Redis idempotency
     -> inventory-remediation -> idempotent inventory release
     -> payment-remediation   -> idempotent full/partial refund
     -> saga-remediation-escalation (rejected or failed actions)
```

Automation is deliberately limited to terminal failures whose history explicitly contains `Rollback not executed for inventory` or `Rollback not executed for payment`. Commands are capped at three actions, automated refunds are capped at R$ 500.00 and 100% of the captured payment, critical-risk decisions require a human, and action identifiers are retained in Redis for 30 days. Diagnostic-cache loss falls back to RAG; idempotency-store loss blocks execution and escalates because duplicate financial actions are unsafe.
